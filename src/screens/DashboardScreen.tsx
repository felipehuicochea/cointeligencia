...
const DashboardScreen: React.FC = () => {
  ...
  const getStrategyLabel = (strategy: string): string => {
    ...
    if (strategyLower === 'intraday' || strategyLower === 'intradía') {
      return t('settings.strategyIntraday');
    } else if (strategyLower === 'multientry' || strategyLower === 'multientrada' || strategyLower.includes('sartp')) {
      return t('settings.strategyMultientry');
    }
    return strategy; // Return original if unknown
  };

  // Filter alerts by enabled strategies and get last 3
  const filteredAlerts = history.alerts.filter(alert => {
    const alertStrategy = alert.strategy.toLowerCase();
    return config.enabledStrategies.some(s => s.toLowerCase() === alertStrategy);
  });
          ...
          <View style={styles.alertsHeader}>
            ...
            <View style={styles.alertsHeaderRight}>
              <Text style={styles.alertsCount}>
                {t('dashboard.alertsCount', { count: recentAlerts.length, total: filteredAlerts.length })}
              </Text>
              ...
              <View key={alert.id} style={styles.alertItem}>
                <View style={styles.alertHeader}>
                  <View style={styles.symbolContainer}>
                    ...
                    <Text style={styles.strategyText}>{getStrategyLabel(alert.strategy)}</Text>
                  </View>
                  <Chip
