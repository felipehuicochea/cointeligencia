...
class CredentialValidationService {
  ...
  // Coinex API validation - Updated to V2 API
  private async validateCoinexCredentials(apiKey: string, apiSecret: string): Promise<ValidationResult> {
    try {
      ...
      // CoinEx V2 API signature: HMAC-SHA256(method + path + body + timestamp, secret)
      // For GET: method + path + timestamp (no body)
      const message = `${method}${path}${body}${timestamp}`;
      const signature = this.generateCoinexV2Signature(message, apiSecret);
      
      // Updated to V2 endpoint - V1 was discontinued on September 25, 2024
      // Coinex V2 uses headers instead of query parameters
      const response = await fetch('https://api.coinex.com/v2/assets/spot/balance', {
        method: 'GET',
        headers: {
          'X-COINEX-KEY': apiKey,
  ...
  // Coinex V2 signature generation
  private generateCoinexV2Signature(message: string, secret: string): string {
    // Coinex V2 uses HMAC-SHA256: timestamp + method + path + body
