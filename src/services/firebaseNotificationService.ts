...
class FirebaseNotificationService {
  ...
  async addNotificationListener(callback: (message: any) => void): Promise<() => void> {
    console.log('[FirebaseNotificationService] Setting up foreground message listener...');
    const unsubscribe = messaging().onMessage(async remoteMessage => {
      console.log('[FirebaseNotificationService] Received foreground message:', JSON.stringify(remoteMessage, null, 2));
      console.log('[FirebaseNotificationService] Message data:', remoteMessage.data);
      console.log('[FirebaseNotificationService] Message notification:', remoteMessage.notification);
      callback(remoteMessage);
    });

    console.log('[FirebaseNotificationService] Foreground listener set up');
    return unsubscribe;
  }

  async addNotificationResponseListener(callback: (response: any) => void): Promise<() => void> {
    console.log('[FirebaseNotificationService] Setting up background/quit message listeners...');
    const unsubscribe = messaging().onNotificationOpenedApp(remoteMessage => {
      console.log('[FirebaseNotificationService] Notification opened app:', JSON.stringify(remoteMessage, null, 2));
      console.log('[FirebaseNotificationService] Message data:', remoteMessage.data);
      callback(remoteMessage);
    });
    ...
    messaging()
      ...
      .then(remoteMessage => {
        if (remoteMessage) {
          console.log('[FirebaseNotificationService] App opened from notification:', JSON.stringify(remoteMessage, null, 2));
          console.log('[FirebaseNotificationService] Initial notification data:', remoteMessage.data);
          callback(remoteMessage);
        } else {
          console.log('[FirebaseNotificationService] No initial notification found');
        }
      })
      .catch(error => {
        console.error('[FirebaseNotificationService] Error getting initial notification:', error);
      });

    console.log('[FirebaseNotificationService] Background/quit listeners set up');
    return unsubscribe;
  }
