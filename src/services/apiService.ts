...
class ApiService {
  ...
  async updateDeviceToken(credentials: { email: string; deviceId: string; fcmToken: string }) {
    try {
      console.log('[apiService] Updating device token:', {
        email: credentials.email,
        deviceId: credentials.deviceId ? 'provided' : 'missing',
        fcmToken: credentials.fcmToken ? `${credentials.fcmToken.substring(0, 20)}...` : 'missing',
        fcmTokenLength: credentials.fcmToken?.length
      });
      ...
      if (response.ok) {
        const result = await response.json();
        console.log('[apiService] Device token updated successfully:', result);
        return result;
      } else {
        const errorData = await response.json().catch(() => ({}));
        console.error('[apiService] Device token update failed:', response.status, errorData);
